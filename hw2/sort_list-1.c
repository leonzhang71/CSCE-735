...
#define MAX_LIST_SIZE   1073741824 
...
//void sort_list(int q) {
    ...
    */
    ...
    // add Barrier 1
    int barrier1 = pthread_barrier_wait(&barrier);
...
if (DEBUG) print_list(list, list_size); 
	...
	//for (my_id = 0; my_id < num_threads; my_id++) {
        ...
        // add Barrier 2
        int barrier2 = pthread_barrier_wait(&barrier);
    ...
    // add Barrier 3 
    int barrier3 = pthread_barrier_wait(&barrier);
...
int main(int argc, char *argv[]) {
    ...
    //**********************************************************
    ptr = (int *) malloc((num_threads+1) * sizeof(int));        // ptr records num_threads+1 info
...
//***********************************************************************************
    ...
    int barrier_check = pthread_barrier_init(&barrier, NULL, num_threads);
    if (barrier_check) printf("ERROR: Unable to initialize barrier!\n");
...
// Thread creation and launching
    for (int idx = 0; idx < num_threads; idx++){
        thread_id[idx] = idx;
        thread_create = pthread_create(&threads[idx], &attr, sort_list, &thread_id[idx]);
        if (thread_create) printf("Error in thread creation. Thread # %d\n", idx);
    }
