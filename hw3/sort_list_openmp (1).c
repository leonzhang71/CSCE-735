...
void sort_list(int q) {
    ...
    //remove and add to another place(line 149)
    //
    /*
...
#pragma omp parallel for \
        ...
        shared(list, ptr, num_threads) \
        private(my_list_size)
    for (my_id = 0; my_id < num_threads; my_id++) {
...
#pragma omp parallel for \
        default(none) \
        num_threads(num_threads) \
        shared(list, ptr, num_threads, q, level, np, list_size, work) \
        private(i,my_id)
	for (my_id = 0; my_id < num_threads; my_id++) {
...
#pragma omp parallel for \
        default(none) \
        num_threads(num_threads) \
        shared(list, ptr, num_threads, work) \
        private(i,my_id)
	for (my_id = 0; my_id < num_threads; my_id++) {
